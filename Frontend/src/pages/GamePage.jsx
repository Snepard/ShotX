...
// --- SVG Icon for Pause Button (No changes) ---
const PauseIcon = () => (
...
const GamePage = () => {
      ...
      projectilesRef.current.forEach((projectile, pIndex) => {
        ...
        if (distProj - enemy.radius - projectile.radius < 1) {
          ...
          if (enemy.radius - 10 > 5) {
            ...
            setTimeout(() => projectilesRef.current.splice(pIndex, 1), 0);
          } else {
            setScore(s => s + 50);
            setTimeout(() => { enemiesRef.current.splice(eIndex, 1); projectilesRef.current.splice(pIndex, 1); }, 0);
          }
        }
      });
    });
  ...
  const handleStartGame = () => { init(); animate(); spawnEnemies(); setGameState('playing'); };
  ...
  // REMOVED: navigateHome function is no longer needed
